...
This repo packages [Kubo](https://github.com/ipfs/kubo) (the reference Go implementation of IPFS) for StartOS.
...
IPFS runs the `ipfs/kubo` image pinned via `dockerTag` in `startos/manifest/index.ts`.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `ipfs/kubo:v<new version>`.
